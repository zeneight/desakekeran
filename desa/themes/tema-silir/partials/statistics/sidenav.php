    ...
    <div class="accordion-item bg-white dark:bg-dark-secondary border border-gray-200 dark:border-dark-primary">
      <h4 class="accordion-header mb-0" id="heading-<?= $statistik['title'] ?>">
        <button
          ...
          <i class="ti <?= $statistik['icon'] ?> mr-2"></i> <?= $statistik['title'] ?>
        </button>
      </h4>
          ...
          <ul class="divide-y dark:divide-dark-primary">
            <?php foreach($statistik['submenu'] as $submenu) : ?>
              ...
              <?php if($this->web_menu_model->menu_aktif($stat_slug)) : ?>
                <li id="statistik_13"><a href="<?= site_url($submenu['slug']) ?>" class="px-5 py-2 block <?= site_url($submenu['slug']) === current_url() ? 'bg-primary text-white' : 'hover:cursor-pointer hover:text-primary dark:text-white' ?>"><?= $submenu['title'] ?></a></li>
              <?php endif ?>
            <?php endforeach ?>
          </ul>
        </div>
